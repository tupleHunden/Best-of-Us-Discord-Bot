...
async def on_member_join(member):
    ...
    await member.dm_channel.send(
        f'Hello {member.name}, welcome to Best of Us!  You should see our #server-info, #announcements, #suggestions, '
        f'and #welcome-mat channels upon joining. '
        f'Check out #server-info first, it will guide you through some information about us and how the server works. '
        f' When you feel comfortable with the rules, check out #welcome-mat to get your roles set up.  Just press '
        f'whichever emoji describes you best. '
...
@bot.command(name='bah',
             help='This command will post a link to the current Basic Allowance for Housing (BAH) calculator.')
...
async def info(ctx):
    embed = discord.Embed(title='Best of Us',
                          description='Best of Us is a community project for Military Veterans in Eve Online from '
                                      'across the world to come together and find a sense of belonging')
...
async def car_loan(ctx):
    principal = float(
        input('Enter the principle of the loan: '))  # Total amount of money you get for the loan to finance.
    interest = float(input('Enter the interest of the loan: '))  # Interest formatted as 7.49 for 7.49% interest.
    length_loan = int(input(
        'Enter the length of the loan in months: '))  # Length of the auto loan, common options are 36, 48, 60, and 72.

    def monthly_loan(principal, interest, length_loan):
        r = ((interest / 100) / 12)  # This takes the number above and converts it to decimal.
        emi = principal * (r * ((1 + r) ** (length_loan))) / (((1 + r) ** (
            length_loan)) - 1)  # This takes the users input and calculates it using Estimated Monthly Installment (
        # EMI) formula.
    ...
    print('Your monthly payment is approximately: $',
          int(monthly))  # This will tell the user what their payment should be, rounded to the nearest whole number.
